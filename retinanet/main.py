...
def parse(args):
    ...
    parser.add_argument(
        ...
        type=str,
        help="Address and port of the master worker",
    ...
    parser_train.add_argument(
        "--annotations", metavar="path", type=str, help="path to COCO style annotations", required=True
    )
    parser_train.add_argument("--images", metavar="path", type=str, help="path to images", default=".")
    parser_train.add_argument(
        "--backbone", action="store", type=str, nargs="+", help="backbone model (or list of)", default=["ResNet50FPN"]
    )
    ...
    parser_train.add_argument(
        "--jitter", metavar="min max", type=int, nargs=2, help="jitter size within range", default=[640, 1024]
    )
    parser_train.add_argument(
        "--iters", metavar="number", type=int, help="number of iterations to train for", default=90000
    )
    parser_train.add_argument(
        "--milestones",
        action="store",
        type=int,
        nargs="*",
        help="list of iteration indices where learning rate decays",
        default=[60000, 80000],
    )
    parser_train.add_argument(
        "--schedule", metavar="scale", type=float, help="scale schedule (affecting iters and milestones)", default=1
    )
    ...
    parser_train.add_argument(
        "--warmup", metavar="iterations", help="numer of warmup iterations", type=int, default=1000
    )
    parser_train.add_argument(
        "--gamma", metavar="value", type=float, help="multiplicative factor of learning rate decay", default=0.1
    )
    parser_train.add_argument("--override", help="override model", action="store_true")
    parser_train.add_argument(
        "--val-annotations", metavar="path", type=str, help="path to COCO style validation annotations"
    )
    ...
    parser_train.add_argument(
        "--val-iters", metavar="number", type=int, help="number of iterations between each validation", default=8000
    )
    ...
    parser_train.add_argument(
        "--augment-brightness", metavar="value", type=float, help="adjust the brightness of the image.", default=0.002
    )
    parser_train.add_argument(
        "--augment-contrast", metavar="value", type=float, help="adjust the contrast of the image.", default=0.002
    )
    parser_train.add_argument(
        "--augment-hue", metavar="value", type=float, help="adjust the hue of the image.", default=0.0002
    )
    parser_train.add_argument(
        "--augment-saturation", metavar="value", type=float, help="adjust the saturation of the image.", default=0.002
    )
    parser_train.add_argument(
        "--regularization-l2", metavar="value", type=float, help="L2 regularization for optim", default=0.0001
    )
    parser_train.add_argument(
        "--rotated-bbox", help="detect rotated bounding boxes [x, y, w, h, theta]", action="store_true"
    )
    parser_train.add_argument(
        "--anchor-ious",
        metavar="value value",
        type=float,
        nargs=2,
        help="anchor/bbox overlap threshold",
        default=[0.4, 0.5],
    )
    parser_train.add_argument(
        "--absolute-angle", help="regress absolute angle (rather than -45 to 45 degrees.", action="store_true"
    )
    ...
    parser_infer.add_argument(
        "--annotations", metavar="annotations", type=str, help="evaluate using provided annotations"
    )
    parser_infer.add_argument(
        "--output", metavar="file", type=str, help="save detections to specified JSON file", default="detections.json"
    )
    ...
    parser_infer.add_argument(
        "--rotated-bbox", help="inference using a rotated bounding box model", action="store_true"
    )
    ...
    parser_export.add_argument(
        "--batch", metavar="size", type=int, help="max batch size to use for TensorRT engine", default=2
    )
    parser_export.add_argument(
        "--full-precision", help="export in full instead of half precision", action="store_true"
    )
    ...
    parser_export.add_argument(
        ...
        metavar="size",
        type=int,
        help="number of batches to use for int8 calibration",
        default=10,
    )
    parser_export.add_argument(
        "--calibration-images",
        metavar="path",
        type=str,
        help="path to calibration images to use for int8 calibration",
        default="",
    )
    parser_export.add_argument(
        "--calibration-table",
...
def load_model(args, verbose=False):
    ...
    if args.command == "train" and (not os.path.exists(args.model) or args.override):
        ...
        model = Model(
            backbones=args.backbone, classes=args.classes, rotated_bbox=args.rotated_bbox, anchor_ious=args.anchor_ious
        )
        ...
        if verbose:
            print("Loading model from {}...".format(os.path.basename(args.model)))
...
def worker(rank, args, world, model, state):
    ...
    if args.command == "train":
        train.train(
            ...
            args.images,
            args.annotations,
            args.val_images or args.images,
            args.val_annotations,
            args.resize,
            args.max_size,
            args.jitter,
            args.batch,
            int(args.iters * args.schedule),
            args.val_iters,
            not args.full_precision,
            args.lr,
            args.warmup,
            [int(m * args.schedule) for m in args.milestones],
            args.gamma,
            is_master=(rank == 0),
            world=world,
            use_dali=args.with_dali,
            metrics_url=args.post_metrics,
            logdir=args.logdir,
            verbose=(rank == 0),
            rotate_augment=args.augment_rotate,
            augment_free_rotate=args.augment_free_rotate,
            augment_brightness=args.augment_brightness,
            augment_contrast=args.augment_contrast,
            augment_hue=args.augment_hue,
            augment_saturation=args.augment_saturation,
            regularization_l2=args.regularization_l2,
            rotated_bbox=args.rotated_bbox,
            absolute_angle=args.absolute_angle,
        )
        ...
        if args.int8:
            ...
            if os.path.isdir(args.calibration_images):
                ...
                if len(calibration_files) >= args.calibration_batches * args.batch:
                    ...
                    print(
                        "Only found enough images for {} batches. Continuing anyway...".format(
