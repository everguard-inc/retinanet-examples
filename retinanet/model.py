...
from .box import (
    decode,
    generate_anchors,
    generate_anchors_rotated,
    nms,
    nms_rotated,
    snap_to_anchors,
    snap_to_anchors_rotated,
)
...
class Model(nn.Module):
    ...
    def __init__(
        ...
        self.cls_criterion = FocalLoss(ignore_index=-1)
    ...
    def forward(self, x, rotated_bbox=None):
        ...
        for cls_head, box_head in zip(cls_heads, box_heads):
            ...
            decoded.append(
                decode(cls_head, box_head, stride, self.threshold, self.top_n, self.anchors[stride], self.rotated_bbox)
            )
    ...
    def export(
        ...
        if not self.rotated_bbox:
            ...
            anchors = [
                generate_anchors_rotated(stride, self.ratios, self.scales, self.angles)[0].view(-1).tolist()
                for stride in self.strides
            ]
